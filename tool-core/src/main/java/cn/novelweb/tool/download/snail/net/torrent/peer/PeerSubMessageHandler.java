...
import cn.novelweb.tool.download.snail.config.PeerConfig.Type;
...
import cn.novelweb.tool.download.snail.pojo.session.PeerConnectSession;
...
import cn.novelweb.tool.download.snail.utils.BitfieldUtils;
...
//		this.peerConnect.release();
	...
	private void exchangeBitfield() {
		...
		if(this.peerSession.supportFastExtensionProtocol()) { // 支持FAST扩展
			final BitSet pieces = this.torrentSession.pieces(); // 已下载Pieces
			if(pieces.isEmpty()) {
				this.haveNone();
				return;
			}
			...
			if(this.torrentSession.completed()) {
				final BitSet allPieces = this.torrentSession.allPieces(); // 所有Pieces
