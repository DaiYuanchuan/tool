...
public final class FileUtils {
	...
	static {
		// 图片文件
		FILE_TYPE_EXT.put(FileType.IMAGE, Arrays.asList(
			"bmp", "cdr", "gif", "ico", "jpeg", "jpg", "png", "psd", "svg", "webp"
		));
		// 视频文件
		FILE_TYPE_EXT.put(FileType.VIDEO, Arrays.asList(
			"3gp", "avi", "flv", "m3u", "m3u8", "mkv", "mov", "mp4", "mpeg", "mvb", "rm", "rmvb", "wmv"
		));
		// 音频文件
		FILE_TYPE_EXT.put(FileType.AUDIO, Arrays.asList(
			"aac", "flac", "m4a", "mp3", "ogg", "wav", "wma"
		));
		// 脚本文件
		FILE_TYPE_EXT.put(FileType.SCRIPT, Arrays.asList(
			"asp", "bat", "c", "cmd", "cpp", "h", "java", "js", "jsp", "php", "py", "sh", "sql"
		));
		// 种子文件
		FILE_TYPE_EXT.put(FileType.TORRENT, Collections.singletonList(
				"torrent"
		));
		// 压缩文件
		FILE_TYPE_EXT.put(FileType.COMPRESS, Arrays.asList(
			"7z", "bz2", "dmg", "gz", "iso", "jar", "rar", "tar", "z", "zip"
		));
		// 文档文件
		FILE_TYPE_EXT.put(FileType.DOCUMENT, Arrays.asList(
			"css", "csv", "doc", "docx", "htm", "html", "json", "log", "md", "pdf", "ppt", "pptx", "txt", "wps", "xls", "xlsx", "xml"
		));
		// 安装程序
		FILE_TYPE_EXT.put(FileType.INSTALL, Arrays.asList(
			"apk", "com", "deb", "exe", "pkg", "rpm"
		));
		// 未知文件
		FILE_TYPE_EXT.put(FileType.UNKNOWN, Collections.emptyList());
	}
