...
public final class Torrent implements Serializable {
	...
	private static final Map<String, Integer> readNodes(List<Object> nodes) {
		...
		List<LinkedHashMap<String,Object>> stream = nodes.stream()
				.map(value -> {
					final List<?> values = (List<?>) value;
					if(values.size() == 2) {
						...
						if(StringUtils.isNumeric(host)) {
							...
							return new LinkedHashMap<String, Object>() {{
								put(NetUtils.intToIP(Integer.parseInt(host)), NetUtils.portToInt(port.shortValue()));
							}};
						} else {
							// 字符串
							return new LinkedHashMap<String, Object>() {{
								put(host, port.intValue());
							}};
						}
					} else {
		...
		for (LinkedHashMap<String,Object> stringObjectLinkedHashMap : stream) {
			stringObjectLinkedHashMap.forEach((k, v) -> stringIntegerMap.put(k,Integer.valueOf(v.toString())));
		}
