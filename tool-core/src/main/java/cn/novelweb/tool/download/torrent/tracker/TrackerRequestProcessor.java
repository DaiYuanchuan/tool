...
import cn.novelweb.tool.download.torrent.common.protocol.AnnounceRequestMessage;
import cn.novelweb.tool.download.torrent.common.protocol.TrackerMessage.ErrorMessage;
import cn.novelweb.tool.download.torrent.common.protocol.TrackerMessage.MessageValidationException;
import cn.novelweb.tool.download.torrent.common.protocol.http.HTTPAnnounceRequestMessage;
import cn.novelweb.tool.download.torrent.common.protocol.http.HTTPAnnounceResponseMessage;
...
public class TrackerRequestProcessor {
  ...
  public void process(final String uri, final String hostAddress, RequestHandler requestHandler)
    ...
    try {
      ...
      serveError(HttpStatus.BAD_REQUEST, mve.getMessage(), requestHandler);
      return;
    }
    ...
    if (!myAcceptForeignTorrents && torrent == null) {
      ...
      serveError(HttpStatus.BAD_REQUEST, ErrorMessage.FailureReason.UNKNOWN_TORRENT, requestHandler);
      return;
    }
    ...
    try {
      ...
      serveError(HttpStatus.BAD_REQUEST, ErrorMessage.FailureReason.INVALID_EVENT, requestHandler);
      return;
    }
  ...
  private void writeEmptyResponse(HTTPAnnounceRequestMessage announceRequest, RequestHandler requestHandler) throws IOException {
    ...
    try {
      ...
      requestHandler.serveResponse(HttpStatus.OK.value(), HttpStatus.OK.getReasonPhrase(), announceResponse.getData());
    } catch (Exception e) {
      serveError(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage(), requestHandler);
    }
  }
...
//			String uri = request.getAddress().toString();
  ...
  private void serveError(HttpStatus status, HTTPTrackerErrorMessage error, RequestHandler requestHandler) throws IOException {
    requestHandler.serveResponse(status.value(), status.getReasonPhrase(), error.getData());
  }
   ...
   */
  private void serveError(HttpStatus status, String error, RequestHandler requestHandler) throws IOException {
   ...
   */
  private void serveError(HttpStatus status, ErrorMessage.FailureReason reason, RequestHandler requestHandler) throws IOException {
